...
def set_cn_axes_options(cn_axes, x, cn, kwargs):
    ...
    if kwargs.get("ylabel", None) != None:
        ...
        cn_axes.set_ylabel("Copy number")

def plot_cn(cn_axes, x, cn, vert=[-100], genes=["ATRX"], chrom="Chromosome", sample="testSamp"):
    """Plot copy number"""
    cn_axes.plot(x, cn, 'o', markersize=1, color = 'black', alpha = .4)
    for xc, gen  in zip(vert,genes):
        plt.axvline(x=xc,color="blue")
        plt.text(xc+1,0,gen,color="blue")
    plt.text(0.5, 1.81, sample+": "+chrom,
        horizontalalignment='center',
        #fontsize=20,
        transform = cn_axes.transAxes)
...
def plot_sv_diagram(x, cn, fusions, outfile, vert=[-100], genes=["ATRX"],chrom="testChrom",sample="testSample", **kwargs):
    ...
    plot_cn(cn_axes, x, cn, vert, genes, chrom, sample)
